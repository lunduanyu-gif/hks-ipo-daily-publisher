...
def validate_images(image_paths: List[str], no_compress: bool = False) -> List[str]:
    """验证图片文件是否存在并进行压缩"""
    valid_images = []
...
def compress_image(image_path: str, max_size_kb: int = 200, max_width: int = 1080) -> str:
    """使用 ImageMagick 压缩图片以符合小红书发布规范"""
    try:
        # 确定输出路径
        ...
        compressed_path = os.path.join(
            ...
            f"{basename}_compressed{ext}"
        )
        
        # 使用 ImageMagick 调整宽度并压缩质量
        # -resize 1080x 保持宽高比
        # -quality 85 压缩质量
        # -strip 移除元数据
        cmd = [
        ...
        if result.returncode != 0:
            print(f"⚠️ ImageMagick 压缩失败: {result.stderr}")
            return image_path
        
        # 检查文件大小，如果仍超过限制则进一步压缩
        ...
        if compressed_size > max_size_kb:
            quality = 70
            while compressed_size > max_size_kb and quality > 50:
                cmd = [
                    'convert', image_path,
                    '-resize', f'{max_width}x',
                    '-quality', str(quality),
        ...
        if compressed_size < original_size:
            print(f"  📦 图片压缩: {os.path.basename(image_path)} ({original_size:.1f}KB → {compressed_size:.1f}KB)")
        
        return compressed_path
        
    except Exception as e:
        print(f"⚠️ 图片压缩失败 {image_path}: {e}")
        return image_path
...
def validate_tags(tags: List[str]) -> bool:
    ...
    # 小红书标签规范：5-10个标签，每个标签不超过10个字符
    if len(tags) < 5:
        print(f"⚠️ 标签数量不足: {len(tags)}个，建议5-10个")
        return False
    if len(tags) > 10:
        print(f"⚠️ 标签数量过多: {len(tags)}个，保留前10个")
        tags = tags[:10]
    
    for tag in tags:
        if len(tag) > 10:
            print(f"⚠️ 标签过长: {tag}，截断至10字符")
            tag = tag[:10]
...
'''
    ...
    # 验证正文内容
    if not args.desc or len(args.desc.strip()) == 0:
        print("❌ 错误: 正文内容不能为空")
        sys.exit(1)
    ...
    if args.tags:
        # 去除 # 符号，统一处理
        clean_tags = [tag.strip('#').strip() for tag in args.tags]
        if validate_tags(clean_tags):
            args.tags = clean_tags
            # 直接在 desc 末尾添加 #话题（小红书会自动识别）
            tags_str = format_tags(args.tags)
            args.desc = f"{args.desc}\n\n{tags_str}"
            print(f"✅ 已添加标签: {tags_str}")
            # 不再使用 topics 参数，直接放正文里
    ...
    # 验证图片
    valid_images = validate_images(args.images, args.no_compress)
